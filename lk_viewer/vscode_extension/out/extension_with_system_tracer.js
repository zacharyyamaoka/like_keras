...
async function reloadCurrentSystem() {
    ...
    try {
        ...
        if (!result.error) {
            ...
            vscode.window.setStatusBarMessage(`✅ System reloaded: ${result.components_count} components`, 3000);
        }
    }
    catch (error) {
...
function setupSystemWebviewMessaging(webview) {
...
async function findComponentDefinition(componentType) {
    // Use VS Code's workspace symbol search
    const symbols = await vscode.commands.executeCommand('vscode.executeWorkspaceSymbolProvider', componentType);
...
//# sourceMappingURL=extension_with_system_tracer.js.map