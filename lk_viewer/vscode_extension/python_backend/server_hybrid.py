...
async def root():
    return {
        ...
        "mode": MODE,
        "workspace": (
            WORKSPACE_ROOT if MODE == "integrated" else "N/A (receiving code via API)"
        ),
    }
...
async def parse_integrated():
    ...
    print(
        f"[Backend] Built graph: {len(graph['nodes'])} nodes, {len(graph['edges'])} edges",
        file=sys.stderr,
    )
...
async def parse_standalone(submission: CodeSubmission):
    ...
    return {"mode": "standalone", "files_count": len(files), "graph": graph}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(
            f"[Backend] Send code via: POST http://localhost:8765/parse",
            file=sys.stderr,
        )
