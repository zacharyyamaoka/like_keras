...
async def parse_workspace():
    ...
    print(f"[Backend] Built graph with {len(graph['nodes'])} nodes and {len(graph['edges'])} edges", file=sys.stderr)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(f"[Backend] No workspace provided, using: {WORKSPACE_ROOT}", file=sys.stderr)
