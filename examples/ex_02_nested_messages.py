...
if __name__ == "__main__":
    ...
    obs = MultiModalObservation(
        camera=CameraObservation(
            image=np.random.rand(480, 640, 3), width=640, height=480, timestamp=1.23
        ),
        lidar=LidarObservation(
            points=np.random.rand(1000, 3), num_points=1000, timestamp=1.23
        ),
        robot_state=RobotState(
    ...
    print(
        f"  - camera still works: {partial_obs.camera.width}x{partial_obs.camera.height}"
    )
