...
def system_to_mermaid(system: "System", rankdir: str = "TB") -> str:
    ...
    for conn in system.graph.connections:
        ...
        if (
            src_comp
            and tgt_comp
            and src_comp in component_ids
            and tgt_comp in component_ids
        ):
...
def plot_system(
    ...
    html_content = MERMAID_TEMPLATE.format(title=html_title, mermaid_code=mermaid_code)
...
def launch_interactive_viewer(
    ...
    layout_components = [
        ...
        html.Div(
            [
                cyto.Cytoscape(
                    ...
                    },  # Full height minus header and panel
        ...
        html.Div(
            ...
            children=[html.Div(id="selected-info", style={"padding": "15px"})],
            style={
                "position": "fixed",
                "bottom": "0",
                "left": "0",
    ...
    if live_update:
        layout_components.append(
            dcc.Interval(
                id="update-interval", interval=update_interval_ms, n_intervals=0
            )
        )
    ...
    @app.callback(Output("system-graph", "layout"), Input("layout-dropdown", "value"))
    def update_layout(layout_name):
        return {"name": layout_name, "directed": True}

    # Callback to show/hide info panel and update content
    @app.callback(
        [Output("info-panel-container", "style"), Output("selected-info", "children")],
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        html.Strong(
                                            "Ports: ", style={"marginRight": "10px"}
                                        ),
                                        html.Span(
                                            f"⬇️ {data.get('num_inputs', 0)} inputs",
                                            style={
                                                "marginRight": "15px",
                                                "color": "#3498db",
                                            },
                                        ...
                                        html.Span(
                                            f"⬆️ {data.get('num_outputs', 0)} outputs",
                                            style={"color": "#e74c3c"},
                                        ),
                                    ],
                                    style={"display": "inline-block"},
    ...
    if live_update:
        ...
        @app.callback(
            Output("system-graph", "elements"), Input("update-interval", "n_intervals")
        )
        def update_graph(n):
