...
def from_config_dict(
    config_class: Type[T], config_dict: dict, is_flat: bool = False, sep: str = "."
) -> T:
    """
    ...
    for component_id, component_dict in config_dict.items():
        if component_id in config_types:
            ...
            result[component_id] = from_config_dict(
                config_class, component_dict, is_flat=False  # Already converted above
            )
        else:
...
class ConfigMixin:
    ...
    @classmethod
    def from_dict(
        cls: Type[T], config_dict: dict, is_flat: bool = False, sep: str = "."
    ) -> T:
