...
class IkFrameOffset(FrameTransformer):
    ...
    def offset_matrix_vectorized(
        self, T_frame_id_to_target_link: np.ndarray, frame_id: str, target_link: str
    ) -> np.ndarray:
        """
...
class MockIkFrameOffset(MockFrameTransformer):
    ...
    def __init__(
        self,
        pin_model: PinRobotModel | None = None,
    ...
    def offset_matrix_vectorized(
        ...
        assert (
            T_frame_id_to_target_link.shape[1] == 4
            and T_frame_id_to_target_link.shape[2] == 4
        )
    ...
    def offset_matrix_vectorized_split(
        ...
        assert (
            R_frame_id_to_target_link.ndim == 3
            and R_frame_id_to_target_link.shape[1] == 3
            and R_frame_id_to_target_link.shape[2] == 3
        )
...
if __name__ == "__main__":
    ...
    T_result = ik_frame_offset.offset_matrix_vectorized(
        T_universe_to_tool0, "universe", "tool0"
    )
    print(f"✅ Success: universe -> {ik_frame_offset.base_link}")
    print(
        f"   Input shape: {T_universe_to_tool0.shape}, Output shape: {T_result.shape}"
    )
    ...
    try:
        ...
        _ = ik_frame_offset.offset_matrix_vectorized(
            T_test, "universe", "not_a_real_link"
        )
