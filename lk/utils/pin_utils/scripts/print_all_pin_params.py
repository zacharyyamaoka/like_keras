...
interia_l1 = pin.Inertia(
    m1,
    np.array([l1 / 2, 0.0, 0.0]),
    np.diag([0.0, m1 * (l1**2) / 12, m1 * (l1**2) / 12]),
)

interia_point_mass = pin.Inertia(payload, np.array([l1, 0.0, 0.0]), np.zeros((3, 3)))
...
# Add link 1
X1 = pin.SE3(np.eye(3), np.array([l1, 0.0, 0.0]))
parent_id = joint_id1
...
C = pin.nonLinearEffects(model, data, q, v) - pin.computeGeneralizedGravity(
    model, data, q
)
