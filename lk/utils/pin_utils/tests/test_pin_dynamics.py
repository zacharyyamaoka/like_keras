...
try:
    ...
    print(
        "BAM_DESCRIPTIONS_PATH and UR_DESCRIPTION_PATH not found, make sure bam_descriptions is installed"
    )
...
def ur5e_model():
    ...
    return PinRobotModel.from_xacro(
        xacro_file, xacro_args, abs_package_dirs, resolve_packages=True
    )
...
def test_one_dof(m, l, one_dof_base_xacro):
    ...
    pin_model = PinRobotModel.from_xacro(
        xacro_file, xacro_args, abs_package_dirs, resolve_packages=True
    )
    ...
    for theta in [
        ...
        print(
            f"m={m}, l={l}, theta: {theta:.2f}, RNEA: {tau[0]:.3f}, Hand Calc: {torque:.3f}, diff: {diff:.3f}"
        )
        if VIZ:
...
def test_six_dof(ur5e_model):
    ...
    for i in range(1, pin_model.model.njoints):  # Skip root joint
        ...
        print(f"    mass: {mass}")
        print(
            f"    lever_arm: abs({link_pos[0]:.3f}) + abs({com[0]:.3f}) = {lever_arm:.3f}"
        )
    ...
    for theta in [
        ...
        for joint_name, payload in payloads.items():
            torque -= (
                payload["mass"] * g * payload["lever_arm"] * np.cos(theta)
            )  # COS as arm starts along x axis
        ...
        print(
            f"theta: {theta:.2f}, RNEA: {tau[joint_2_idx]:.3f}, Hand Calc: {torque:.3f}, diff: {diff:.3f}"
        )
