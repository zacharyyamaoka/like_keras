...
def test_box_obstacle_collision_detected(pin_collision: PinCollision) -> None:
    ...
    pin_collision.add_box_obstacle(
        name="test_box", size=0.3, pose=box_pose  # 30cm cube
    )

    # Check collision at neutral position
    assert pin_collision.is_collision(
        q_neutral
    ), "Box should collide with robot in neutral position"
...
def test_box_obstacle_no_collision_when_disabled(pin_collision: PinCollision) -> None:
    ...
    assert not pin_collision.is_collision(
        q_neutral
    ), "Box should NOT collide when collision pairs disabled"
...
def test_cylinder_obstacle_collision_detected(pin_collision: PinCollision) -> None:
    ...
    assert pin_collision.is_collision(
        q_neutral
    ), "Cylinder should collide with robot in neutral position"


def test_cylinder_obstacle_no_collision_when_disabled(
    pin_collision: PinCollision,
) -> None:
    ...
    assert not pin_collision.is_collision(
        q_neutral
    ), "Cylinder should NOT collide when collision pairs disabled"
...
def test_mesh_obstacle_collision_detected(pin_collision: PinCollision) -> None:
    ...
    assert pin_collision.is_collision(
        q_neutral
    ), "Mesh should collide with robot in neutral position"
...
def test_mesh_obstacle_no_collision_when_disabled(pin_collision: PinCollision) -> None:
    ...
    assert not pin_collision.is_collision(
        q_neutral
    ), "Mesh should NOT collide when collision pairs disabled"
...
def test_all_obstacles_together(pin_collision: PinCollision) -> None:
    ...
    if os.path.exists(mesh_path):
        ...
        mesh_pose[2, 3] = 0.163
        pin_collision.add_mesh_obstacle(
            "mesh", mesh_path, mesh_pose, scale=(1.0, 1.0, 1.0)
        )

    # At least one should be in collision
    assert pin_collision.is_collision(
        q_neutral
    ), "At least one obstacle should collide with robot"
...
def test_rectangular_box_obstacle(pin_collision: PinCollision) -> None:
    ...
    box_pose[2, 3] = 0.163
    pin_collision.add_box_obstacle(
        name="rect_box", size=(0.5, 0.3, 0.2), pose=box_pose  # Different dimensions
    )

    # Check collision
    assert pin_collision.is_collision(
        q_neutral
    ), "Rectangular box should collide with robot"
...
def test_update_obstacle_pose(pin_collision: PinCollision) -> None:
    ...
    assert not pin_collision.is_collision(
        q_neutral
    ), "Box should not collide when far away"
    ...
    assert pin_collision.is_collision(
        q_neutral
    ), "Box should collide after moving to robot position"
