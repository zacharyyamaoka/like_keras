...
def T_target_link_to_ik_tip(arm: RobotDescription):
    ...
    T_target_link_to_ik_tip = pin_model.get_transform_between_frames(DIFF_IK_TIP, IK_TIP, q_update=pin_model.get_q_neutral())
...
def T_frame_id_to_base_link(arm: RobotDescription):
    pin_model = PinRobotModel.from_robot_description(arm)
    T_frame_id_to_base_link = pin_model.get_transform_between_frames(DIFF_BASE_LINK, BASE_LINK, q_update=pin_model.get_q_neutral())
...
def test_offset_different_frame_id_and_target_link(
    ...
    if should_be_equal:
        ...
        assert not np.allclose(T_input, result), \
...
def test_offset_vectorized_consistency(
    ...
    single_results = [frame_transformer.transform(
        T, 
        input_frame=frame_id, 
        input_link=target_link,
        output_frame=BASE_LINK,
...
def test_offset_matrix_vectorized_split_consistency(frame_transformer: FrameTransformer, frame_id: str, target_link: str, should_be_equal: bool):
