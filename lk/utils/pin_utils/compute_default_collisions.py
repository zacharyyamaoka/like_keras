...
def compute_default_collisions(
    ...
    if robot_model.collision_model is None:
        raise ValueError("PinRobotModel must have collision_model loaded for collision detection")
    ...
    if verbose:
        ...
        print(f"Total collision pairs to check: {len(collision_model_full.collisionPairs)}")
    ...
    for joint_id in range(1, model.njoints):  # Skip universe joint (id=0)
        ...
        if parent_joint_id > 0 and child_frame_id > 0:
            ...
            if parent_link in link_to_geom_indices and child_link in link_to_geom_indices:
                ...
                if pair_key not in disabled_link_pairs:
                    ...
                    collision_pairs_to_disable.append(CollisionPair(link1, link2, "Adjacent"))
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', message='This function has been marked as deprecated')
        pin.computeCollisions(model, data, collision_model_full, collision_data_full, q_zero, False)
    ...
    for pair_idx, pair in enumerate(collision_model_full.collisionPairs):
        ...
        if result.isCollision():
            ...
            if pair_key not in disabled_link_pairs:
                ...
                collision_pairs_to_disable.append(CollisionPair(link1, link2, "Default"))
    ...
    for sample_idx in tqdm(
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', message='This function has been marked as deprecated')
            pin.computeCollisions(model, data, collision_model_full, collision_data_full, q_random, False)
    ...
    for i, link1 in enumerate(link_names):
        for j in range(i + 1, n_links):
            ...
            if collision_cnt == num_samples:
                ...
                collision_pairs_to_disable.append(CollisionPair(link1_sorted, link2_sorted, "Always"))
                always_count += 1
                ...
                collision_pairs_to_disable.append(CollisionPair(link1_sorted, link2_sorted, "Never"))
                never_count += 1
    ...
    if verbose:
        print(f"\n{'='*60}")
        print("SUMMARY:")
        print(f"  Adjacent pairs  : {len([p for p in collision_pairs_to_disable if p.reason == 'Adjacent'])}")
        print(f"  Default pairs   : {len([p for p in collision_pairs_to_disable if p.reason == 'Default'])}")
