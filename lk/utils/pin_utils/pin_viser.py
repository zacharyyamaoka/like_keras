...
class PinViser():
    ...
    @classmethod
    def from_robot_description(cls, rd: 'RobotDescription', port="", color=None, auto_add_controls=True):
        robot_model = PinRobotModel.from_robot_description(rd)
        
        # Get display link names from robot description
        display_frame_names = rd.links.get_display_link_names() if hasattr(rd, 'links') else []
        ...
        return cls(robot_model, port=port, color=color, auto_add_controls=auto_add_controls, 
                   display_frame_names=display_frame_names, joint_positions=joint_positions)
    ...
    @classmethod
    def from_xacro(cls, xacro_path: str, xacro_args: dict, mesh_package_dirs: str | list[str], port="", color=None, auto_add_controls=True):
        robot_model = PinRobotModel.from_xacro(xacro_path, xacro_args, mesh_package_dirs)
        return cls(robot_model, port=port, color=color, auto_add_controls=auto_add_controls)
    
    @classmethod
    def from_urdf(cls, urdf_path: str, mesh_package_dirs: str | list[str], port="", color=None, auto_add_controls=True):
        robot_model = PinRobotModel.from_urdf(urdf_path, mesh_package_dirs)
        return cls(robot_model, port=port, color=color, auto_add_controls=auto_add_controls)

    def __init__(self, robot_model: PinRobotModel, port="", color=None, auto_add_controls=True, display_frame_names=None, joint_positions:dict[str, list[float]]=None):
        """
        ...
        self.viz = ViserVisualizer(self.robot_model.model, self.robot_model.collision_model, self.robot_model.visual_model)
        ...
        self._display_frame_names = set(display_frame_names) if display_frame_names else None
        self._show_display_frames_only = True  # By default, filter by display frames if provided
    ...
    def add_robot_control_sliders(self):
        ...
        with self.server.gui.add_folder("Joint position control"):
            ...
            if self._joint_positions:
                ...
                position_dropdown.on_update(lambda _: self._load_joint_position(position_dropdown.value))
            ...
            for i, joint_name in enumerate(self.robot_model.get_joint_names()):
                ...
                slider.on_update(  # When sliders move, we update the URDF configuration.
                    lambda _: self.display(np.array([slider.value for slider in slider_handles]))
                )
    ...
    def add_inertial_control(self):
        ...
        with self.server.gui.add_folder("Inertial Properties"):
            ...
            inertia_checkbox.on_update(lambda _: self._toggle_inertia(inertia_checkbox.value))
            ...
            opacity_slider.on_update(lambda _: self._update_inertial_opacity(opacity_slider.value))
    ...
    def _update_inertial_opacity(self, opacity: float):
        ...
        print(f"[INERTIAL CONTROL] Inertial opacity updated to {opacity} (applies to inertia boxes only)")
    ...
    def _update_com_visualization(self):
        ...
        for i, inertia in enumerate(self.robot_model.model.inertias):
            ...
            if frame_name in self._com_handles:
                ...
                print(f"[COM] {link_name}: mass={mass:.4f}kg, radius={radius:.4f}m, pos={com_pos_world}")
    ...
    def _update_inertia_visualization(self):
        ...
        for i, inertia in enumerate(self.robot_model.model.inertias):
            ...
            if not np.isclose(det, 1.0, atol=1e-2):
                print(f"[WARNING] Invalid rotation matrix for {link_name}, det={det:.3f}, skipping inertia visualization")
                continue
            
            # Convert to quaternion using Pinocchio (wxyz format for viser)
            try:
                quat_pinocchio = pin.Quaternion(R_box_world)
                quat_wxyz = quat_pinocchio.coeffs()[[3, 0, 1, 2]]  # Pinocchio uses xyzw, convert to wxyz
            except Exception as e:
                print(f"[WARNING] Could not convert rotation to quaternion for {link_name}: {e}")
                continue
            ...
            if box_name in self._inertia_handles:
                ...
                print(f"[INERTIA] {link_name}: mass={mass:.4f}kg, dims={box_lengths}, det={det:.3f}")
    ...
    def add_visual_control(self):
        ...
        with self.server.gui.add_folder("Visual Appearance"):
            ...
            visual_checkbox.on_update(lambda _: self._toggle_visual(visual_checkbox.value))
            ...
            alpha_slider.on_update(lambda _: self._update_visual_alpha(alpha_slider.value))
            
            # Color picker (RGB)
            color_picker = self.server.gui.add_rgb(
                label="Color",
                initial_value=(128, 128, 255) if self._visual_color is None else tuple((np.array(self._visual_color) * 255).astype(int)),
            )
            color_picker.on_update(lambda _: self._update_visual_color(np.array(color_picker.value) / 255.0))
            ...
            wireframe_checkbox.on_update(lambda _: self._update_visual_wireframe(wireframe_checkbox.value))
    ...
    def add_collision_control(self):
        ...
        with self.server.gui.add_folder("Collision"):
            ...
            collision_checkbox.on_update(lambda _: self._toggle_collision(collision_checkbox.value))
            ...
            opacity_slider.on_update(lambda _: self._update_collision_opacity(opacity_slider.value))
    ...
    def _update_visual_color(self, color: np.ndarray):
        ...
        # Format: [R, G, B, Alpha]
        color_rgba = [float(color[0]), float(color[1]), float(color[2]), float(self._visual_alpha)]
    ...
    def _update_visual_wireframe(self, wireframe: bool):
        ...
        for geom_id, geometry_object in enumerate(self.robot_model.visual_model.geometryObjects):
        ...
        print(f"[VISUAL CONTROL] Wireframe mode {'enabled' if wireframe else 'disabled'}")
    ...
    def add_frame_control(self):
        ...
        with self.server.gui.add_folder("Frame Control"):
            ...
            if self._display_frame_names is not None:
                ...
                display_only_checkbox.on_update(lambda _: self._toggle_display_frames_only(display_only_checkbox.value))
            ...
            link_frames_checkbox.on_update(lambda _: self._toggle_link_frames(link_frames_checkbox.value))
            ...
            joint_frames_checkbox.on_update(lambda _: self._toggle_joint_frames(joint_frames_checkbox.value))
            ...
            link_labels_checkbox.on_update(lambda _: self._toggle_link_frame_labels(link_labels_checkbox.value))
            ...
            joint_labels_checkbox.on_update(lambda _: self._toggle_joint_frame_labels(joint_labels_checkbox.value))
            ...
            font_mode_dropdown.on_update(lambda _: self._update_label_font_mode(font_mode_dropdown.value))
            ...
            label_size_slider.on_update(lambda _: self._update_frame_label_size(label_size_slider.value))
            ...
            depth_test_checkbox.on_update(lambda _: self._update_label_depth_test(depth_test_checkbox.value))
    ...
    def _update_label_depth_test(self, enabled: bool):
        ...
        print(f"[FRAME CONTROL] Label depth test {'enabled' if enabled else 'disabled'}")
    ...
    def _toggle_display_frames_only(self, enabled: bool):
        ...
        print(f"[FRAME CONTROL] Display frames only filter {'enabled' if enabled else 'disabled'}")
    ...
    def _update_frame_visibility(self):
        ...
        for frame in self.robot_model.model.frames:
            ...
            if viser_frame_name in self.viz.frames:
                ...
                should_show = (is_link and self._show_link_frames) or (is_joint and self._show_joint_frames)
                
                # Apply display frame filter if enabled
                if should_show and self._display_frame_names is not None and self._show_display_frames_only:
    ...
    def _update_frame_size(self, size: float):
        ...
        for frame in self.robot_model.model.frames:
            ...
            if viser_frame_name in self.viz.frames:
                ...
                viser_frame.origin_radius = size * 0.075  # Slightly larger than axes radius
    ...
    def _update_frame_labels(self):
        ...
        for i, frame in enumerate(self.robot_model.model.frames):
            ...
            should_show_label = (is_link and self._show_link_frame_labels) or (is_joint and self._show_joint_frame_labels)
    ...
    def _clear_frame_labels_by_type(self, is_link: bool = False, is_joint: bool = False):
    ...
    def _update_frame_label_size(self, size: float):
        ...
        print(f"[FRAME CONTROL] Label size updated to {size} ({self._label_font_mode} mode)")
