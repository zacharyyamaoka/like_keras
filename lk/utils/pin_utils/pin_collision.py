...
class PinCollision:
    ...
    def from_robot_description(cls, robot_description: 'RobotDescription', verbose=False):
        ...
        return cls(robot_model, srdf_xml=robot_description.get_srdf_xml(), verbose=verbose)


    def __init__(self, robot_model: PinRobotModel, srdf_path: Optional[str] = None, srdf_xml: Optional[str] = None, verbose=False):
        """
        DescriptionArgs:
        ...
        if self.collision_model is None:
            raise ValueError("PinModel must have collision_model loaded for collision checking")
        ...
        self.geometry_name_to_id = {
            geom.name: idx for idx, geom in enumerate(self.collision_model.geometryObjects)
        }

        # Save full list BEFORE removal for inspection
        all_pairs = [(pair.first, pair.second,
        ...
        if self.verbose:
            print(f"[Collision] Pairs after SRDF filtering: {len(active_name_pairs)} active of {len(all_pairs)} total")
            print("[Collision] Collision Pairs (with SRDF tag):")

            for i, (first_idx, second_idx, name1, name2) in enumerate(all_pairs):
                tag = "ACTIVE" if (name1, name2) in active_name_pairs or (name2, name1) in active_name_pairs else "REMOVED"
                print(f"  {i+1:03d}. {name1} <-> {name2}   [{tag}]")

        else:
            print(f"[Collision] Pairs after SRDF filtering: {len(active_name_pairs)} active of {len(all_pairs)} total")

    def get_collision_pairs(self, filter_links: Optional[list[str]] = None) -> list[tuple[str, str]]:
        ...
        filtered_pairs = [
            (name1, name2) for name1, name2 in all_pairs
            if name1 in filter_links or name2 in filter_links
        ]
        
        print(f"Total Collision Pairs: {len(all_pairs)}, Matching Pairs: {len(filtered_pairs)}/{len(all_pairs)}")
    ...
    def enable_collision_pair(self, name_a: str, name_b: str) -> bool:
        ...
        existing_pairs = {(pair.first, pair.second) for pair in self.collision_model.collisionPairs}
    ...
    def plot_collision_matrix(
        ...
        if robot_links is not None:
            all_links = {link for link in all_links if any(rlink in link for rlink in robot_links)}
        ...
        ax.imshow(collision_matrix, cmap='Greens', interpolation='nearest', aspect='auto')
        ...
        white_patch = mpatches.Patch(color='white', label='No collision', edgecolor='gray')
    ...
    def get_collision_info(self, q: np.ndarray, break_on_collision: bool = False) -> tuple[bool, list[tuple[str, str]]]:
        """
        ...
        for k, pair in enumerate(self.collision_model.collisionPairs):
            ...
            if result.isCollision():
                ...
                if self.verbose:
                    print(f"[Collision] Collision detected: {geom1_name} <-> {geom2_name}")
    ...
    def _add_collision_pairs(self, obstacle_id: int, obstacle_name: str, 
        ...
        if check_link_names is None:
            ...
            for geom_obj in self.collision_model.geometryObjects:
                if geom_obj.name != obstacle_name and geom_obj.name not in self.obstacles:
            ...
            for link_name in check_link_names:
                ...
                for geom_obj in self.collision_model.geometryObjects:
                    ...
                    if geom_obj.name == link_name or geom_obj.name.startswith(f"{link_name}_"):
                        try:
                            link_id = self.collision_model.getGeometryId(geom_obj.name)
                            pair = pin.CollisionPair(obstacle_id, link_id)
                            self.collision_model.addCollisionPair(pair)
                            matched = True
                            if self.verbose:
                                print(f"[Collision] Added collision pair: {obstacle_name} <-> {geom_obj.name}")
                        except Exception as e:
                            if self.verbose:
                                print(f"[Collision] Warning: Could not add pair for {geom_obj.name}: {e}")
                
                if not matched and self.verbose:
                    print(f"[Collision] Warning: No geometry found matching '{link_name}'")
        
        # Recreate collision_data after adding new pairs
        self.collision_data = pin.GeometryData(self.collision_model)

    def add_box_obstacle(self, name: str, size: float | tuple[float, float, float], 
                         pose: np.ndarray | pin.SE3,
                         check_link_names: Optional[list[str]] = None,
                         color: Optional[np.ndarray] = None) -> int:
        """
        ...
        if name in self.obstacles:
            raise ValueError(f"Obstacle '{name}' already exists. Use update_obstacle_pose() to move it.")
        ...
        if self.verbose:
            print(f"[Collision] Added box obstacle '{name}' (ID: {box_id}) at position {box_placement.translation}")
        
        return box_id
    
    def add_cylinder_obstacle(self, name: str, radius: float, height: float,
                              pose: np.ndarray | pin.SE3,
                              check_link_names: Optional[list[str]] = None,
                              color: Optional[np.ndarray] = None) -> int:
        """
        ...
        if self.verbose:
            print(f"[Collision] Added cylinder obstacle '{name}' (ID: {cylinder_id}) at position {cylinder_placement.translation}")
        
        return cylinder_id
    
    def add_mesh_obstacle(self, name: str, mesh_path: str, 
                          pose: np.ndarray | pin.SE3,
        ...
        if self.verbose:
            print(f"[Collision] Added mesh obstacle '{name}' (ID: {mesh_id}) at position {mesh_placement.translation}")
    ...
    def update_obstacle_pose(self, name: str, pose: np.ndarray | pin.SE3):
        ...
        if name not in self.obstacles:
            raise ValueError(f"Obstacle '{name}' not found. Use add_box_obstacle(), add_cylinder_obstacle(), or add_mesh_obstacle() to create it first.")
        ...
        if self.verbose:
            print(f"[Collision] Updated obstacle '{name}' to position {new_placement.translation}")
