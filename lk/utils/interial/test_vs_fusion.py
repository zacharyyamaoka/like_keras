...
def compare_box_dimensions(
    computed: np.ndarray, reference: np.ndarray, component_name: str = ""
) -> bool:
    ...
    print(
        f"  Dims: ref={reference} vs comp={computed}, diff={diff:.6f} m, {'✓' if match else '✗'}"
    )
...
def compare_orientations(
    R_computed: np.ndarray, R_reference: np.ndarray, component_name: str = ""
) -> bool:
    """Compare rotation matrices and assert on failure."""
    match = rotation_is_close(
        R_computed, R_reference, theta_tol=np.radians(10.0), verbose=True
    )
...
def test_inertia_box_vs_fusion360(
    visualize: bool = False, visualize_component: str = None
):
    """
    ...
    for txt_file in properties_files:
        ...
        print(
            f"COM: [{props.com_m[0]:.6f}, {props.com_m[1]:.6f}, {props.com_m[2]:.6f}] m"
        )
        ...
        try:
            box_lengths, R_principal = equivalent_inertia_box(
                props.mass_kg, inertia_matrix
            )

            print(f"\n✓ Equivalent Inertia Box:")
            print(
                f"  Dimensions: [{box_lengths[0]:.6f}, {box_lengths[1]:.6f}, {box_lengths[2]:.6f}] m"
            )
            print(f"  Principal axes rotation:")
            for i in range(3):
                print(
                    f"    [{R_principal[i, 0]:+.6f}, {R_principal[i, 1]:+.6f}, {R_principal[i, 2]:+.6f}]"
                )
        ...
        if props.bbox_m:
            ...
            print(
                f"  Fusion 360 Physical BBox: [{fusion_bbox[0]:.6f}, {fusion_bbox[1]:.6f}, {fusion_bbox[2]:.6f}] m"
            )
            print(
                f"  Inertia Box Dimensions:   [{box_lengths[0]:.6f}, {box_lengths[1]:.6f}, {box_lengths[2]:.6f}] m"
            )
            ...
            print(
                f"  Dims: ref={fusion_bbox} vs comp={box_lengths}, diff={diff:.6f} m, {'✓' if match else '✗ (expected for rotated objects)'}"
            )
        ...
        if props.name in obb_data:
            ...
            print(
                f"    Extents: [{obb_extents[0]:.6f}, {obb_extents[1]:.6f}, {obb_extents[2]:.6f}] m"
            )
            print(
                f"    Center: [{obb_center[0]:.6f}, {obb_center[1]:.6f}, {obb_center[2]:.6f}] m"
            )
            print(f"  Inertia box:")
            print(
                f"    Extents: [{box_lengths[0]:.6f}, {box_lengths[1]:.6f}, {box_lengths[2]:.6f}] m"
            )
            print(
                f"    Center: [{props.com_m[0]:.6f}, {props.com_m[1]:.6f}, {props.com_m[2]:.6f}] m"
            )

            # Compare poses using matrix_is_close
            pose_match = matrix_is_close(
                inertia_box_transform,
                obb_transform,
                pos_tol=1e-3,
            ...
            if not pose_match:
                pos_diff = np.linalg.norm(
                    inertia_box_transform[:3, 3] - obb_transform[:3, 3]
                )
            ...
            print(
                f"  Extents comparison: {'✓ PASS' if extents_match else '✗ FAIL'} (sum diff: {extents_diff:.6f} m)"
            )
        ...
        if visualize and (
            visualize_component is None or visualize_component in props.name
        ):
            ...
            visualize_boxes_o3d(
                props, box_lengths, R_principal, obb_extents, obb_transform
            )
...
def visualize_boxes_o3d(
    props,
    box_lengths: np.ndarray,
    R_principal: np.ndarray,
    obb_extents: np.ndarray = None,
    obb_transform: np.ndarray = None,
):
    """
    ...
    if props.bbox_m:
        aa_box = o3d.geometry.OrientedBoundingBox(
            center=props.com_m, R=np.eye(3), extent=props.bbox_m
        )
        aa_box.color = [0, 1, 0]  # Green
        geometries.append(aa_box)
        print(
            f"[Green] Fusion 360 AA-BBox: [{props.bbox_m[0]:.4f}, {props.bbox_m[1]:.4f}, {props.bbox_m[2]:.4f}] m"
        )

    # Box 2: Inertia OBB (red) at COM with principal axes
    inertia_obb = o3d.geometry.OrientedBoundingBox(
        center=props.com_m, R=R_principal, extent=box_lengths
    )
    inertia_obb.color = [1, 0, 0]  # Red
    geometries.append(inertia_obb)
    print(
        f"[Red]   Inertia OBB:        [{box_lengths[0]:.4f}, {box_lengths[1]:.4f}, {box_lengths[2]:.4f}] m"
    )
    ...
    if obb_extents is not None and obb_transform is not None:
        ...
        fusion_obb = o3d.geometry.OrientedBoundingBox(
            center=obb_center, R=obb_rotation, extent=obb_extents
        )
        fusion_obb.color = [0, 0, 1]  # Blue
        geometries.append(fusion_obb)
        print(
            f"[Blue]  Fusion 360 OBB:     [{obb_extents[0]:.4f}, {obb_extents[1]:.4f}, {obb_extents[2]:.4f}] m"
        )
