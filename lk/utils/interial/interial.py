...
def equivalent_inertia_box(
    ...
    if np.any(eigenvalues <= 0):
        raise ValueError(f"Inertia matrix must be positive definite, got eigenvalues {eigenvalues}")
    
    if np.linalg.det(inertia_matrix) <= 0:
        raise ValueError(f"Inertia matrix determinant must be positive, got {np.linalg.det(inertia_matrix)}")
...
if __name__ == "__main__":
    ...
    print(f"Computed box dimensions: [{box_lengths[0]:.3f}, {box_lengths[1]:.3f}, {box_lengths[2]:.3f}] m")
