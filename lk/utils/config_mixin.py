...
import os
import yaml
from dacite import from_dict  # convert nested dataclasses to dicts properly, standard asdict() doesn't work
...
@dataclass
class ConfigMixin:
    _metadata: Dict[str, Any] = field(default_factory=lambda: {
        "version": "1.0",
        "created_by": os.popen("git config --get user.name").read().strip() or "unknown",
    ...
    def replace_types_with_file_paths(self, target_types: tuple[type, ...], get_file_path_method: str = "save_yaml_file") -> Any:
    ...
    def from_json_file(cls, file_path='', verbose=False):
        ...
        if not file_path:
            temp_cls = cls() # create a temporary instance to get the _save_dir and _file_name
            ...
            file_path = get_latest_file(temp_cls._save_dir, temp_cls._file_name, file_ending=".json", verbose=verbose)

        with open(file_path, "r") as f:
            data = json.load(f)
        if not isinstance(data, dict):
            raise ValueError("ConfigMixin file must be a JSON object at the top level.")
        return from_dict(data_class=cls, data=data)  # dataclass takes care of filling in fields
    ...
    def from_yaml_file(cls, file_path='', verbose=False):
        ...
        if not file_path:
            temp_cls = cls() # create a temporary instance to get the _save_dir and _file_name
            file_path = get_latest_file(temp_cls._save_dir, temp_cls._file_name, file_ending=".yaml", verbose=verbose)
        ...
        return from_dict(data_class=cls, data=data) # Use dacite, instead which handles nested dataclasses/lists/etc.
