...
def plane_to_matrix(a: float, b: float, c: float) -> np.ndarray:
    """Convert plane coefficients z = ax + by + c into homogeneous transform."""
    normal = np.array([-a, -b, 1.0], dtype=float)
    normal /= np.linalg.norm(normal)

    up = np.array([0.0, 0.0, 1.0], dtype=float)

    y_axis = np.cross(normal, up)
    ...
    transform[:3, 3] = np.array([0.0, 0.0, c])
    return transform


def matrix_to_plane(matrix: np.ndarray) -> tuple[float, float, float]:
    """Convert homogeneous transform back into plane coefficients."""
    normal = matrix[:3, 2]
    a, b, _ = -1.0 * normal / normal[2]
    c = matrix[:3, 3][2]
    return float(a), float(b), float(c)


def calculate_plane(points: np.ndarray) -> tuple[float, float, float]:
    """Least-squares plane fit for point cloud."""
    X = points[:, 0]
    Y = points[:, 1]
    Z = points[:, 2]
    ...
    return float(a_val), float(b_val), float(c_val)
    ...
    def fit_ransac(self, points_array,
        ...
        for i in range(number_of_iterations):
            ...
            if use_density_normalization:
                # Note: density normalization not currently supported
                # Falling back to simple count
