...
ThresholdsType = TypeVar('ThresholdsType', ValueThresholds, CounterThresholds, PercentThresholds, BoxChartThresholds)
...
class UnifiedDiagnosticConfig(Generic[ThresholdsType]):
    ...
    freq_queue_val: int | float | None = None  # Optional: for EventPeriodTask frequency calculation
    freq_queue_type: str | None = None  # Optional: for EventPeriodTask frequency calculation
...
class UnifiedDiagnosticTask(ABC, DiagnosticTask, Generic[ThresholdsType]):
    ...
    def __init__(self, config: UnifiedDiagnosticConfig[ThresholdsType], name: str = None):
        ...
        self.values = make_queue(self.queue_val, self.queue_type) if self.queue_val > 0 else None
        self.stat = DiagnosticStatusWrapper(level=DiagnosticStatus.STALE, name=self.name, message="__init__")
    ...
    def run(self, stat: DiagnosticStatusWrapper) -> DiagnosticStatusWrapper:
        ...
        for threshold_attr, value, upper_bound in threshold_checks:
            ...
            threshold_status = check_thresholds(threshold_value, value, threshold_attr, upper_bound)
            combined_status.mergeSummary(threshold_status)
            
            # Add to diagnostic output
            stat.add(f"{threshold_attr} [{threshold_value.warn}, {threshold_value.error}]", f"{value:.3f}")
    ...
    def from_config(cls, config_dict: dict, name: str = None):
        ...
        try:
            ...
            raise ImportError("dacite is required for from_config(). Install with: pip install dacite")
        ...
        thresholds = from_dict(
            data_class=threshold_class,
            data=config_dict['thresholds']
        )
...
class BoxChartTask(UnifiedDiagnosticTask[BoxChartThresholds]):
    ...
    """
    def __init__(self, config: UnifiedDiagnosticConfig[BoxChartThresholds], name: str = None):
...
class ValueTask(UnifiedDiagnosticTask[ValueThresholds]):
    ...
    def __init__(self, config: UnifiedDiagnosticConfig[ValueThresholds], name: str = None):
        super().__init__(config, name)
        self.value = 0
...
class CounterTask(UnifiedDiagnosticTask[CounterThresholds]):
    ...
    """
    def __init__(self, config: UnifiedDiagnosticConfig[CounterThresholds], name: str = None):
        super().__init__(config, name)
...
class PercentTask(UnifiedDiagnosticTask[PercentThresholds]):
    ...
    """
    def __init__(self, config: UnifiedDiagnosticConfig[PercentThresholds], name: str = None):
        super().__init__(config, name)
        self.all_time_count = 0
...
class EventPeriodTask(BoxChartTask):
    ...
    def __init__(self, config: UnifiedDiagnosticConfig[BoxChartThresholds], name: str = None):
        ...
        freq_val = config.freq_queue_val if config.freq_queue_val is not None else config.queue_val
        freq_type = config.freq_queue_type if config.freq_queue_type is not None else config.queue_type

        self.count = 0
        self.last_period = 1e6  # large default value to start, to avoid division by zero!
