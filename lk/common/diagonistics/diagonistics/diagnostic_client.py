...
class DiagonisticClient:
    ...
    def __init__(self, node: Node, lifespan_sec: int = 60, use_ros_clock=False):
        ...
        self.subscription = self.node.create_subscription(
            DiagnosticArray, "/diagnostics", self.topic_cb, 1
        )
    ...
    def topic_cb(self, msg: DiagnosticArray):
        ...
        self.stamp_queue.append(
            self.msg_stamp_sec(msg)
        )  # better to append now then calculate each time you check...
...
if __name__ == "__main__":
    ...
    msg_1.header.stamp = Time_from_sec(
        0.0
    )  # stamp internal is done based on recivied time! so this won't matter...
    msg_1.status = [
        DiagnosticStatusWrapper(
            level=DiagnosticStatus.ERROR, name="status_1", message="error_status"
        )
    ]
    ...
    msg_2.status = [
        DiagnosticStatusWrapper(
            level=DiagnosticStatus.OK, name="status_1", message="ok_status"
        )
    ]
