...
def verify_robot_description(
    ...
    _verify_joint_transforms(robot_description, robot_model, stable_transform_tol, verbose)
...
def _verify_names(robot_description: 'RobotDescription', robot_model: PinRobotModel, verbose: bool) -> None:
    """
...
def _verify_named_positions(robot_description: 'RobotDescription', robot_model: PinRobotModel, verbose: bool) -> None:
    """
    ...
    for pos_name in position_names:
        try:
            ...
            q_ordered = robot_model.order_q_by_name(actuated_joint_names, np.array(q_values))
            
            # Verify no NaN values
            if np.any(np.isnan(q_ordered)):
                errors.append(f"Position '{pos_name}' resulted in NaN values after ordering")
                continue
    ...
    print(f"  ✓ All {len(position_names)} named positions verified: {list(position_names)}")
...
def _verify_joint_transforms(
    ...
    for joint in specified_joints:
        try:
            ...
            if joint.is_fixed:
                ...
                actual_transform = robot_model.get_transform_between_frames(parent_frame, child_frame)
    ...
    print(f"  ✓ All {rotating_count + fixed_count} joint transforms verified ({rotating_count} rotating, {fixed_count} fixed)")


def _verify_tags(robot_description: 'RobotDescription', expected_tags: list[str], verbose: bool) -> None:
    """
    ...
    for joint_name in joint_transform_dict.keys():
        try:
            ...
            if joint.is_fixed:
                ...
                parent_frame = joint.transform.header.frame_id
                child_frame = joint.transform.child_frame_id
                joint_transforms[joint_name] = robot_model.get_transform_between_frames(parent_frame, child_frame)
                print_v(f"    - Saved transform for {joint_name} (fixed)")
            else:
                # Rotating joints - use joint placement
                joint_transforms[joint_name] = robot_model.get_rotating_joint_placement(joint_name)
