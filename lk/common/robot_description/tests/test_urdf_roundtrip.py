...
def test_round_trip_real_robots(robot_name, urdf_path):
    ...
    try:
        original_urdf = yourdfpy.URDF.load(
            urdf_path, load_meshes=False, build_scene_graph=True
        )
    ...
    print(
        f"  Loaded: {len(robot_desc.links.entities)} links, {len(robot_desc.joints.entities)} joints"
    )
    ...
    try:
        ...
        pytest.fail(
            f"Failed to parse exported {robot_name} URDF with yourdfpy: {e}\nSaved to {debug_path}"
        )
