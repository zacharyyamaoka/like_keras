...
from bam.descriptions import (
    BAM_MESH_PACKAGE_PATH, BAM_DESCRIPTIONS_PATH, UrdfInfo, RobotDescription, 
    RobotInfo, ArmArgs, JointDescription, LinkDescription, Joints, Links, TAGS
)
...
class DummyDHJoints(Joints):
    joint_1: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    joint_2: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    joint_3: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    joint_4: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    joint_5: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    joint_6: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))

@dataclass
class DummyDHLinks(Links):
    base_link: LinkDescription = field(default_factory=lambda: LinkDescription(display=True, is_frame=True, tags=[TAGS.base_mount, TAGS.ik_base]))
    ...
    link_3: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))
    link_4: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))
    link_5: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))
    ee_link: LinkDescription = field(default_factory=lambda: LinkDescription(is_frame=True))
    tool0: LinkDescription = field(default_factory=lambda: LinkDescription(display=True, is_frame=True, tags=[TAGS.ik_tip, TAGS.arm_to_hand_mount]))
    tool0_flip: LinkDescription = field(default_factory=lambda: LinkDescription(is_frame=True, tags=[TAGS.ik_tip_flip]))
...
class DummyDH(RobotDescription):
    ...
    def from_dh_params(cls, dh_params: DHParams, reflect: int = 1):
        return cls(
            args=DummyDHArgs(dh=dh_params, reflect=reflect)
        )

if __name__ == '__main__':
    rd = DummyDH.from_dh_params(DHParams(d1=0.2, a2=-0.5, a3=-0.5, d4=0.2, d5=0.15, d6=0.15), reflect=1)
    print(rd)
