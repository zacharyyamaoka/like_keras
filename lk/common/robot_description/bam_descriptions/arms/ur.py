...
from bam.descriptions import (
    JointDescription, UrdfInfo, SrdfInfo, RobotInfo, LinkDescription,
    RobotDescription, Joints, Links, JointPositions, ArmArgs, TAGS
)
...
class URJoints(Joints):
    shoulder_pan_joint: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    shoulder_lift_joint: JointDescription = field(default_factory=lambda: JointDescription(initial_position=-1.5707))
    elbow_joint: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    wrist_1_joint: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    wrist_2_joint: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))
    wrist_3_joint: JointDescription = field(default_factory=lambda: JointDescription(initial_position=0.0))

@dataclass
class URLinks(Links):
    base_link_ur: LinkDescription = field(default_factory=lambda: LinkDescription(display=True, is_frame=True, tags=[TAGS.base_mount, TAGS.ik_base]))  # Frame only - Display by default
    ...
    forearm_link: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))  # Enable collision
    wrist_1_link: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))  # Enable collision
    wrist_2_link: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))  # Enable collision
    wrist_3_link: LinkDescription = field(default_factory=lambda: LinkDescription(scene_obj_collision=True))  # Enable collision
    ee_link: LinkDescription = field(default_factory=lambda: LinkDescription(is_frame=True))  # Frame only
    tool0: LinkDescription = field(default_factory=lambda: LinkDescription(display=True, is_frame=True, tags=[TAGS.ik_tip, TAGS.arm_to_hand_mount]))  # Frame only - Display by default (TCP)
    tool0_flip: LinkDescription = field(default_factory=lambda: LinkDescription(is_frame=True, tags=[TAGS.ik_tip_flip]))  # Frame only
...
class URSrdfInfo(SrdfInfo):
    xacro_path: str = "/home/bam/public_ws/src/Universal_Robots_ROS2_Driver/ur_moveit_config/srdf/ur.srdf.xacro"  # Optinally can set
...
class URArgs(ArmArgs):
    ...
    ik_sol_preference: list[IKSolPreference] = field(default_factory=lambda: [
    ...
    def make_UR5e(
        ...
        **kwargs
        ...
        args = URArgs(
            ...
            base_xyz=base_xyz,
            base_rpy=base_rpy,
            reach_map_path=BAM_REACH_PATH + "/maps/ur/ur5e/ur5e_table_0.8x1.5x0.8_0.30_180x45x360_09_aug_2025.pkl",
            **kwargs,
    ...
    def make_UR10(
        ...
        **kwargs
        ...
        args = URArgs(
            ...
            base_xyz=base_xyz,
            base_rpy=base_rpy,
            reach_map_path=BAM_REACH_PATH + "/maps/ur/ur10/ur10_table_0.8x1.5x0.8_0.30_180x45x360_09_aug_2025.pkl",
            **kwargs,
        )
