...
def test_arm_conveyor_ur():
    ...
    conveyor = ConveyorDescription(prefix="conveyor_", width=0.4, length=1.5, height=0.05)
    ...
    print(f"\n  Validation:")
    print(f"    ✓ All {len(joint_names)} joint names unique: {len(joint_names) == len(set(joint_names))}")
    print(f"    ✓ All {len(link_names)} link names unique: {len(link_names) == len(set(link_names))}")
...
def test_arm_conveyor_bam():
    ...
    conveyor = ConveyorDescription(prefix="conveyor_", width=0.5, length=2.0, height=0.06)
...
def test_urdf_loading():
    ...
    try:
        ...
        try:
            ...
            print(f"    Note: This might fail if the config file format doesn't match xacro expectations")
        
        # Test arm+conveyor URDF
        print(f"\n  Testing ArmConveyor URDF...")
        arm = UR.make_UR5e(prefix="arm_")
        conveyor2 = ConveyorDescription(prefix="conveyor_", width=0.4, length=1.5, height=0.05)
        arm_conveyor = ArmConveyor(
            arm=arm,
            conveyor=conveyor2,
            arm_mount_xyz=(0.0, 0.0, 0.1)
