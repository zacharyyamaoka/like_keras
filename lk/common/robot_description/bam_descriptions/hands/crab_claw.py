...
class CrabClawUrdfInfo(UrdfInfo):
    macro_path: str = (
        BAM_DESCRIPTIONS_PATH + "/urdf/crab_claw_v1/crab_claw_v1_macro.xacro"
    )
    xacro_path: str = (
        BAM_DESCRIPTIONS_PATH + "/urdf/crab_claw_v1/crab_claw_v1_config.urdf.xacro"
    )
...
class ClawJoints(Joints):
    ...
    eef_joint_1: JointDescription = field(
    ...
    joint_virtual_finger: JointDescription = field(
        default_factory=lambda: JointDescription(
...
class ClawLinks(Links):
    ...
    eef_link_1: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            display=True, scene_obj_collision=True, tags=[TAGS.hand_to_arm_mount]
        )
    )  # Hand base - enable collision
    eef_link_2: LinkDescription = field(
        default_factory=lambda: LinkDescription(scene_obj_collision=True)
    )  # Moving finger - enable collision
    virtual_finger: LinkDescription = field(
        default_factory=lambda: LinkDescription(scene_obj_collision=True)
    )  # Virtual finger - enable collision
    tcp_world: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            is_frame=True, display=True, tags=[TAGS.tcp_world]
        )
    )  # Frame only
    tcp_tool: LinkDescription = field(
        default_factory=lambda: LinkDescription(is_frame=True, tags=[TAGS.tcp_tool])
    )  # Frame only
    floating_tcp_world: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            is_frame=True, display=True, tags=[TAGS.tcp_world_floating]
        )
    )  # Frame only
    crab_claw_center: LinkDescription = field(
...
class CrabClaw(RobotDescription):
    ...
    @classmethod
    def make_bam(
        cls, prefix: Optional[str] = None, disable_floating_tcp: bool = False, **kwargs
    ):
        ...
        args = CrabClawArgs(
            ...
            opening_angles=[
                float(np.deg2rad(point[1])) for point in dataset
