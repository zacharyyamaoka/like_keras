...
class ConveyorUrdfInfo(BamUrdfInfo):
    ...
    xacro_path: str = (
        BAM_DESCRIPTIONS_PATH + "/urdf/conveyor/conveyor_config.urdf.xacro"
    )
...
class ConveyorJoints(Joints):
    ...
    belt_joint: JointDescription = field(
...
class ConveyorLinks(Links):
    ...
    surface_center: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            name="surface_center", scene_obj_collision=False
        )
    )  # No collision with scene objects
    center: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            name="center", scene_obj_collision=False
        )
    )  # No collision with scene objects
    surface_front: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            name="surface_front", scene_obj_collision=False
        )
    )  # No collision with scene objects
    roller_front: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            name="roller_front", scene_obj_collision=False
        )
    )  # No collision with scene objects
    roller_back: LinkDescription = field(
        default_factory=lambda: LinkDescription(
            name="roller_back", scene_obj_collision=False
        )
    )  # No collision with scene objects
...
class ConveyorDescription(RobotDescription):
    ...
    def railing_total_height(self) -> float:
        ...
        return (
            self.args.height
            + self.args.railing_height_above
            + self.args.railing_height_below
        )
