...
def generate_composite_xml_body(description: 'RobotDescription') -> str:
    ...
    links_xml = "\n".join([link.to_xml() for link in description.links.entities.values()])
    
    # Joints XML (each joint already has proper indentation and newlines)
    joints_xml = "\n".join([joint.to_xml() for joint in description.joints.entities.values()])
    ...
    urdf_content = textwrap.dedent(f'''
        <!--
