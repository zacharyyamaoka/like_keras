...
try:
    ...
    raise ImportError("yourdfpy is required for URDF conversion. Install with: pip install yourdfpy")
...
def parse_origin(origin_element) -> tuple[list[float], list[float]]:
    ...
    xyz = list(origin_element.xyz) if hasattr(origin_element, 'xyz') else [0.0, 0.0, 0.0]
    rpy = list(origin_element.rpy) if hasattr(origin_element, 'rpy') else [0.0, 0.0, 0.0]
    
    return xyz, rpy


def yourdfpy_to_transform(parent_link: str, child_link: str, origin) -> TransformStamped:
...
def parse_material_from_yourdfpy(material) -> Material:
    ...
    if hasattr(material, 'texture') and material.texture is not None:
        mat.texture_filename = material.texture.filename if hasattr(material.texture, 'filename') else ""
...
def parse_inertial_from_yourdfpy(inertial) -> InertialProperties:
    ...
    mass = float(inertial.mass.value) if hasattr(inertial, 'mass') and hasattr(inertial.mass, 'value') else 1.0
...
def parse_joint_from_yourdfpy(joint) -> JointDescription:
    ...
    if hasattr(joint, 'axis') and joint.axis is not None:
        axis_list = list(joint.axis.xyz) if hasattr(joint.axis, 'xyz') else [1.0, 0.0, 0.0]
...
def from_yourdfpy_urdf(urdf: 'yourdfpy.URDF', robot_name: Optional[str] = None) -> tuple[list[LinkDescription], list[JointDescription], RobotInfo]:
...
def from_urdf_string(urdf_string: str, robot_name: Optional[str] = None) -> tuple[list[LinkDescription], list[JointDescription], RobotInfo]:
    ...
    """
    import io
    urdf = yourdfpy.URDF.load(io.StringIO(urdf_string), load_meshes=False, build_scene_graph=True)
    return from_yourdfpy_urdf(urdf, robot_name)


def from_urdf_file(urdf_path: str | Path, robot_name: Optional[str] = None) -> tuple[list[LinkDescription], list[JointDescription], RobotInfo]:
...
def geometry_to_urdf_xml(geom: Geometry, indent: str = "      ") -> str:
    ...
    if isinstance(geom, Box):
        ...
        scale_str = ' '.join([f'{v:.6f}' for v in geom.scale]) if geom.scale != [1.0, 1.0, 1.0] else None
...
def inertial_to_urdf_xml(inertial: InertialProperties, indent: str = "    ") -> str:
    ...
    # Inertia
    inert = inertial.inertia
    lines.append(f'{indent}  <inertia ixx="{inert.ixx:.6f}" ixy="{inert.ixy:.6f}" ixz="{inert.ixz:.6f}" '
...
def visual_to_urdf_xml(visual: VisualProperties, indent: str = "    ", name: str = "") -> str:
...
def collision_to_urdf_xml(collision: CollisionProperties, indent: str = "    ", name: str = "") -> str:
...
def link_to_urdf_xml(link: LinkDescription, indent: str = "  ") -> str:
    ...
    if isinstance(link.visual, list):
        for i, visual in enumerate(link.visual):
            vis_name = visual.name if hasattr(visual, 'name') and visual.name else f"visual_{i}"
        ...
        vis_name = link.visual.name if hasattr(link.visual, 'name') and link.visual.name else ""
    ...
    if isinstance(link.collision, list):
        for i, collision in enumerate(link.collision):
            col_name = collision.name if hasattr(collision, 'name') and collision.name else f"collision_{i}"
        ...
        col_name = link.collision.name if hasattr(link.collision, 'name') and link.collision.name else ""
...
def joint_to_urdf_xml(joint: JointDescription, indent: str = "  ") -> str:
    ...
    if joint.type != 'fixed' and joint.limits:
        ...
        lines.append(f'{indent}  <limit lower="{lower:.6f}" upper="{upper:.6f}" '
                    f'effort="{effort:.6f}" velocity="{velocity:.6f}"/>')
    
    # Dynamics
    if joint.physics and (joint.physics.damping != 0.0 or joint.physics.friction != 0.0):
        lines.append(f'{indent}  <dynamics damping="{joint.physics.damping:.6f}" '
...
def to_urdf_string(links: list[LinkDescription], joints: list[JointDescription], robot_name: str = "robot") -> str:
