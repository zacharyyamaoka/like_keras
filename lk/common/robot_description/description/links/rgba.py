...
class RGBA:
    ...
    @classmethod
    def from_numpy(cls, array: np.ndarray, base8: bool = False, bgr: bool = False) -> 'RGBA':
    ...
    @classmethod
    def from_list(cls, values: list[float], base8: bool = False, bgr: bool = False) -> 'RGBA':
        return cls.from_numpy(np.array(values), base8, bgr)

    @classmethod
    def from_tuple(cls, value: tuple[float], base8: bool = False, bgr: bool = False) -> 'RGBA':
        return cls.from_numpy(np.array(list(value)), base8, bgr)


    def to_numpy(self, alpha: bool = True, base8: bool = False, bgr: bool = False) -> np.ndarray:
    ...
    def to_list(self, alpha: bool = True, base8: bool = False, bgr: bool = False) -> list[float]:
        return self.to_numpy(alpha, base8, bgr).tolist()

    def to_tuple(self, alpha: bool = True, base8: bool = False, bgr: bool = False) -> tuple:
    ...
    @classmethod
    def to_gradient_list(cls, start_color: 'RGBA' = None, end_color: 'RGBA' = None, num_points: int = 10, cmap=None) -> list['RGBA']:
        ...
        if start_color is None or end_color is None:
            raise ValueError("start_color and end_color are required when cmap is not provided")
        
        if num_points < 2:
            raise ValueError("num_points must be at least 2 when using start_color and end_color")
    ...
    @classmethod
    def random(cls) -> 'RGBA':
        rgba = np.random.uniform(0.5, 1., 4).tolist()
        return cls(*rgba)
...
if __name__ == "__main__":
    ...
    assert np.allclose(round_trip.to_numpy(base8=True, bgr=False), base8), "base8 round-trip failed"

    # bgr
    rgba = RGBA.from_numpy(arr, base8=False, bgr=False)
    bgr = rgba.to_numpy(base8=False, bgr=True)
    round_trip = RGBA.from_numpy(bgr, base8=False, bgr=True)
    assert np.allclose(round_trip.to_numpy(base8=False, bgr=True), bgr), "bgr round-trip failed"

    # base8 + bgr
    rgba = RGBA.from_numpy(arr, base8=False, bgr=False)
    base8_bgr = rgba.to_numpy(base8=True, bgr=True)
    round_trip = RGBA.from_numpy(base8_bgr, base8=True, bgr=True)
    assert np.allclose(round_trip.to_numpy(base8=True, bgr=True), base8_bgr), "base8+bgr round-trip failed"
    ...
    round_trip = RGBA.from_numpy(base8, base8=True, bgr=False)
    assert np.allclose(round_trip.to_numpy(alpha=False, base8=True, bgr=False), base8), "base8 round-trip failed"

    # bgr
    rgba = RGBA.from_numpy(arr, base8=False, bgr=False)
    bgr = rgba.to_numpy(alpha=False, base8=False, bgr=True)
    round_trip = RGBA.from_numpy(bgr, base8=False, bgr=True)
    assert np.allclose(round_trip.to_numpy(alpha=False, base8=False, bgr=True), bgr), "bgr round-trip failed"

    # base8 + bgr
    rgba = RGBA.from_numpy(arr, base8=False, bgr=False)
    base8_bgr = rgba.to_numpy(alpha=False, base8=True, bgr=True)
    round_trip = RGBA.from_numpy(base8_bgr, base8=True, bgr=True)
    assert np.allclose(round_trip.to_numpy(alpha=False, base8=True, bgr=True), base8_bgr), "base8+bgr round-trip failed"

    # Test gradient
    gradient = RGBA.to_gradient_list(RGBA.from_list([1.0, 0.0, 0.0, 1.0]), RGBA.from_list([0.0, 0.0, 1.0, 1.0]), 5)
    ...
    assert gradient[0].to_tuple() == (1.0, 0.0, 0.0, 1.0), "Gradient should start with the start color"
    assert gradient[-1].to_tuple() == (0.0, 0.0, 1.0, 1.0), "Gradient should end with the end color"
    # Also check that intermediate values are interpolated
    expected_middle = RGBA(0.5, 0.0, 0.5, 1.0).to_tuple()
    assert np.allclose(gradient[2].to_tuple(), expected_middle), "Gradient midpoint color is not as expected"
    print(gradient)
    ...
    assert darkened.r == 0.5 and darkened.g == 0.0 and darkened.b == 0.0, "Darkening failed"
