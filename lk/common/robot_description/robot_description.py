...
# BAM
from bam.utils import combine_dataclass_instances, generate_srdf_from_urdf, ConfigMixin, temp_urdf_file, temp_srdf_file, xml_from_xacro, xml_body_from_macro_xml
        ...
        for instance in named_entities:
            for entity in instance.entities.values():
                if entity.name in combined_dict:
                    raise ValueError(f"Entity name {entity.name} already exists in combined dictionary")
...
class DescriptionTags:
    ...
    # Hand
    tcp_tool: str ="tcp_tool" # similar to tool0 frame, in that Z pointing away from palm
...
class Links(NamedEntities):
    ...
    def find_by_tag(self, tag: str, unique: bool = True) -> LinkDescription | list[LinkDescription]:
        matching_links = [link for link in self if tag in link.tags]

        if unique:
            if len(matching_links) > 1:
                raise ValueError(f"Multiple links: {matching_links} found with tag {tag}")
...
class Joints(NamedEntities):
    ...
    def get_actuated_joint_names(self, prefix=True) -> list[str]:
        return [joint.name if prefix else joint.unprefixed_name for joint in self.get_actuated_joints()]
...
class RobotDescription(ConfigMixin): 
    ...
    points_of_interest: list[PointOfInterest] = field(default_factory=list) # TODO make this easier to look up?

    urdf: UrdfInfo = field(default_factory=UrdfInfo)
    srdf: SrdfInfo = field(default_factory=SrdfInfo)
    config_file: str = ""  # Path to config file, set via info.get_file_path("_config.yaml")

    args: DescriptionArgs = field(default_factory=DescriptionArgs) # Unique args for each robot
    ...
    generate_py_xml: bool = False  # If True, generate XML programmatically from links/joints

    @classmethod
    def from_entities(cls, entities: list[LinkDescription | JointDescription], robot_info: Optional[RobotInfo] = None) -> 'RobotDescription':
    ...
    def combine(descriptions: list['RobotDescription'], entities_to_add: Optional[list[LinkDescription | JointDescription]] = None, robot_info: Optional[RobotInfo] = None) -> 'RobotDescription':
        ...
        joint_positions = JointPositions.combine([description.joint_positions for description in descriptions])
        points_of_interest = [point for description in descriptions for point in description.points_of_interest]
    ...
    def extend(self, descriptions: list['RobotDescription'], entities_to_add: Optional[list[LinkDescription | JointDescription]] = None, robot_info: Optional[RobotInfo] = None) -> 'RobotDescription':
        """Extend this RobotDescription with additional descriptions."""
        return RobotDescription.combine([self] + descriptions, entities_to_add=entities_to_add, robot_info=robot_info)
    ...
    def inspect(self, indent: int = 0) -> None:
        ...
        print(f"{indent_str}  joint_positions: {len(self.joint_positions.joint_names)} joints")
    ...
    def get_body_xml(self) -> str:
        ...
        if self.generate_py_xml:
            ...
            return self.urdf.get_xml_body(self.get_config_file(), resolve_packages=False)
    ...
    def init_joint_positions(self) -> 'RobotDescription':
        ...
        if len(self.joint_positions.joint_names) == 0:
            self.joint_positions.joint_names = self.joints.get_actuated_joint_names(prefix=True)
        if len(self.joint_positions.unprefixed_joint_names) == 0:
            self.joint_positions.unprefixed_joint_names = self.joints.get_actuated_joint_names(prefix=False)
        ...
        if "initial" not in self.joint_positions.positions:
            self.joint_positions.positions["initial"] = [joint.initial_position for joint in actuated_joints]
        ...
        if "zeros" not in self.joint_positions.positions:
            ...
            self.joint_positions.positions["zeros"] = [
                max(0.0, joint.limits.min_position if joint.limits.min_position is not None else -np.pi) 
        ...
        if "middle" not in self.joint_positions.positions:
            ...
            self.joint_positions.positions["middle"] = [
                ((joint.limits.min_position if joint.limits.min_position is not None else -np.pi) + joint.limits.max_position) / 2.0
                for joint in actuated_joints
            ]
        ...
        for joint_name in self.joints.get_actuated_joint_names(prefix=False):
            if joint_name not in self.joint_positions.get_joint_names(prefix=False):
                raise ValueError( f"'{joint_name}' should be in joint_positions.joint_names: {self.joint_positions.joint_names}")
    ...
    def to_xacro_args(self, set_config_file: bool = True) -> dict[str, str]:
        from .format_for_xacro import format_for_xacro # We want to check for the robot description, careful for circular
    ...
    def generate_new_srdf(self, num_samples: Optional[int] = None, verbose: bool = True):
    ...
    def get_srdf_xml(self, resolve_packages: bool = False) -> str:
        ...
        return self.srdf.get_xml(self.to_xacro_args(), resolve_packages=resolve_packages)
