...
class DataPlayback(Component):
    ...
    class Config:
        ...
        rtf: Optional[float] = (
            None  # Real-time factor (alias for rate, takes precedence)
        )
        ...
        for key in self.dataset.keys():
            ...
            try:
                ...
                if not issubclass(msg_type, Msg):
                    print(
                        f"Warning: {key} contains non-Msg data ({msg_type}), skipping port creation"
                    )
                    continue
    ...
    def play_realtime(
        self, rate: Optional[float] = None, blocking: bool = True
    ) -> None:
        """
        ...
        data_start_time = (
            timestamps[self._current_index]
        ...
        while not self.done:
            ...
            target_playback_time = playback_start + (
                elapsed_data_time / self.config.rate
            )
    ...
    def get_current_values(self) -> dict[str, Any]:
        ...
        """
        return {
            key: self.outputs.get_port(key).value
            for key in self.dataset.keys()
...
if __name__ == "__main__":
    ...
    print("\nExample 2: Loop playback")
    playback2 = DataPlayback(dataset=dataset, config=DataPlayback.Config(loop=True))

    print("  Playing 8 steps (5 dataset + 3 looped):")
    for i in range(8):
        playback2.step()
        print(
            f"    Step {i}: index={playback2._current_index-1}, done={playback2.done}"
        )
    ...
    playback4 = DataPlayback(
        dataset=dataset, config=DataPlayback.Config(start_offset=0.2)
    )
