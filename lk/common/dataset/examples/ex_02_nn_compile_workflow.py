...
def collect_training_data(controller: PDController, 
                         n_episodes: int = 100,
                         episode_length: int = 50) -> Dataset:
    """
    ...
    for episode in range(n_episodes):
        ...
        for step in range(episode_length):
            ...
            # Record
            recorder.record('state', state, timestamp=sample_count)
            recorder.record('control', control, timestamp=sample_count)
            sample_count += 1
...
def evaluate_neural_controller(test_states: list[State],
                               introduce_error: bool = False) -> Dataset:
    """
...
def compare_controllers(dataset_original: Dataset,
                       dataset_neural: Dataset,
...
if __name__ == '__main__':
    ...
    test_states = [
        State(position=np.random.uniform(-5, 5), 
    ...
    dataset_neural_perfect = evaluate_neural_controller(test_states, introduce_error=False)
