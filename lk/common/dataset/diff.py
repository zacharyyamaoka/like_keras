    ...
    def summary(self) -> str:
        ...
        if not self.values_match:
            ...
            for key, key_diff in self.key_diffs.items():
                lines.append(f"\n  {key}:")
                lines.append(
                    f"    Length: A={key_diff.length_a}, B={key_diff.length_b}"
                )
                lines.append(
                    f"    Matching samples: {key_diff.n_matching}/{key_diff.length_a}"
                )
...
class KeyDiff:
    ...
    mismatches: list[tuple[int, Any, Any]] = field(
        default_factory=list
    )  # (index, val_a, val_b)


def compare_values(
    val_a: Any, val_b: Any, tol: float = 1e-6
    ...
    # Handle lists/arrays
    if isinstance(val_a, (list, tuple, np.ndarray)) and isinstance(
        val_b, (list, tuple, np.ndarray)
    ):
        ...
        if np.issubdtype(arr_a.dtype, np.number) and np.issubdtype(
            arr_b.dtype, np.number
        ):
    ...
    if isinstance(val_a, (int, float, np.number)) and isinstance(
        val_b, (int, float, np.number)
    ):
...
def diffdiff(
    ...
    """
    result = DiffResult(keys_match=False, timestamps_match=True, values_match=True)
    ...
    for key in common_keys:
        ...
        key_diff = KeyDiff(key=key, length_a=len(data_a), length_b=len(data_b))
...
def assert_datasets_equal(
    dataset_a: Dataset, dataset_b: Dataset, tol: float = 1e-6, message: str = ""
) -> None:
    """
