...
class ComponentLifecycleMixin:
    ...
    def configure(self):
        ...
        if self._lifecycle_state != LifecycleState.UNCONFIGURED:
            raise RuntimeError(f"Cannot configure from state {self._lifecycle_state}")
    ...
    def activate(self):
        ...
        if self._lifecycle_state != LifecycleState.INACTIVE:
            raise RuntimeError(f"Cannot activate from state {self._lifecycle_state}")
    ...
    def deactivate(self):
        ...
        if self._lifecycle_state != LifecycleState.ACTIVE:
            raise RuntimeError(f"Cannot deactivate from state {self._lifecycle_state}")
    ...
    def shutdown(self):
        ...
        """
        if self._lifecycle_state != LifecycleState.INACTIVE:
            raise RuntimeError(f"Cannot shutdown from state {self._lifecycle_state}")
