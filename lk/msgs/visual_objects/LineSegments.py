...
class LineSegments(VisualObject):
    ...
    def from_numpy(
        ...
        assert (
            points.ndim == 3 and points.shape[1] == 2 and points.shape[2] == 3
        ), "points must be a 3D array with shape (N, 2, 3)"
        assert (
            colors.ndim == 3
            and colors.shape[0] == points.shape[0]
            and colors.shape[1] == 2
        ), "colors must be a 3D array with shape (N, 2, 3) or (N, 2, 4)"
        assert colors.shape[2] in (
            3,
            4,
        ...
        return cls(
            points=points_flat, colors=colors_flat, line_width=line_width, name=name
        )
