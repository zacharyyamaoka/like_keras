...
class GraspPath(CompoundVisualObject):
    ...
    # Gripper data
    gripper_opening: list[float] = field(default_factory=list)  # Opening width (m) or angle (rad)
    ...
    def build_model(self):
        ...
        assert len(self.gripper_opening) == len(self.path.pose_list), \
    ...
    def _add_grippers(self):
        ...
        for i, (pose, width) in enumerate(zip(self.path.pose_list, self.gripper_opening)):
    ...
    def from_waypoint_action(
        ...
        if path_props is not None:
            ...
            path.pose_list = action.waypoints # Keep the visual properties, just add the data
            path.build_model()
        else:
...
if __name__ == "__main__":
    ...
    gripper_opening = [0.08, 0.08, 0.02, 0.02, 0.02, 0.08, 0.08]  # Open -> Close -> Open
