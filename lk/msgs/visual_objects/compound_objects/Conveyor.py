...
class Conveyor(CompoundVisualObject):
    ...
    # Data
    T_world_to_surface_center: TransformStamped = field(default_factory=TransformStamped)
    size: tuple[float, float, float] = (0.4, 2.0, 0.05)  # (width, length, height)
    
    # Visual properties
    belt_material: Material = field(default_factory=lambda: Material(
    ...
    def build_model(self):
        ...
        if self.frame_scale > 0.0:
            ...
            frame_surface_center = Frame(
                ...
                axis_length=self.frame_scale * 1.2  # Slightly larger to indicate primary
            )
...
if __name__ == "__main__":
    ...
    conveyor5 = Conveyor(
        ...
        T_world_to_surface_center=TransformStamped.from_xyzrpy([1.0, 1.0, 0], [0, 0, np.pi/4]),
        size=(0.4, 2.0, 0.05)
    )
