...
class ScoreCloud(CompoundVisualObject):
    ...
    def to_numpy(self, alpha: bool = True, base8: bool = False, bgr: bool = False) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    def filter_copy(self, min_score: float = 0.0, max_score: float = 1.0) -> 'ScoreCloud':
    ...
    def _generate_colors(self, scores: list[float]) -> list[RGBA]:
        ...
        try:
            ...
            print(f"Warning: Unknown colormap '{self.colormap}', falling back to 'viridis'")
        ...
        **kwargs
        ...
        if colors is not None:
            if colors.ndim == 2 and colors.shape[1] in (3, 4):
                point_cloud.colors = [
                    RGBA.from_numpy(color) for color in colors
                ]
            else:
        ...
        return cls(
            ...
            colormap=colormap if colors is None else None,  # Only use colormap if no colors
...
if __name__ == "__main__":
    ...
    print(f"  Exported: points {pts_np.shape}, colors {colors_np.shape}, scores {scores_np.shape}")
