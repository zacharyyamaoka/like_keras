...
class PointCloud(VisualObject):
    ...
    @classmethod
    def from_points(cls, points: list[list[float, float, float]], colors: list[RGBA] = [RGBA.grey()], name: str = "") -> 'PointCloud':
    ...
    @classmethod
    def from_numpy(cls, points: np.ndarray, colors: np.ndarray | RGBA, base8: bool = False, bgr: bool = False, name: str = "") -> 'PointCloud':
        points = copy.deepcopy(points)
        # this supports colors with and without alpha!
        assert points.ndim == 2 and points.shape[1] == 3, "points must be a 2D array with shape (N, 3)"
        ...
        if isinstance(colors, RGBA): # Single RGBA object - use for all points
            ...
            assert colors.shape[0] == num_points, "colors must have same length as points"
            assert colors.shape[1] in (3, 4), "colors must have 3 (RGB) or 4 (RGBA) channels"
            colors_flat = []
            for color in colors:
    ...
    def from_rgbd(
        ...
        if downsample > 1:
            ...
            rgb, depth, camera_info = downsample_rgbd(rgb, depth, camera_info, scale=scale)
        ...
        if rgb.dtype == np.uint8:
            colors = rgb.reshape(-1, 3)[valid_mask.reshape(-1)].astype(np.float32) / 255.0
        else:
            colors = rgb.reshape(-1, 3)[valid_mask.reshape(-1)].astype(np.float32)
        
        return cls.from_numpy(points=points, colors=colors, base8=False, bgr=False, name=name)