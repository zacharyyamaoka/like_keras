...
class RandomInt(RandomType):
    ...
    def uniform(cls, low: int, high: int) -> "RandomInt":
        ...
        """
        return cls(
            sample_fn=lambda rng: rng.integers(low, high), min_val=low, max_val=high - 1
        )
    ...
    def binomial(cls, n: int, p: float) -> "RandomInt":
        ...
        """
        return cls(sample_fn=lambda rng: int(rng.binomial(n, p)), min_val=0, max_val=n)
    ...
    @classmethod
    def pareto(
        cls, shape: float, min_val: int = 1, max_val: Optional[int] = None
    ) -> "RandomInt":
...
if __name__ == "__main__":
    ...
    ri_custom = RandomInt.custom(
        lambda rng: int(rng.negative_binomial(n=5, p=0.5)), min_val=0, max_val=50
    ).with_seed(42)
