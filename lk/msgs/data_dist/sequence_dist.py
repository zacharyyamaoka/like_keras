...
class SequenceDist(DataDist):
    ...
    def get_range(self) -> tuple[Optional[dict], Optional[dict]]:
        ...
        min_dict = {"length": length_range[0], "element": element_range[0]}
        max_dict = {"length": length_range[1], "element": element_range[1]}
    ...
    def fixed_length(cls, length: int, element_dist: DataDist) -> "SequenceDist":
        ...
        Args:
            ...
            element_dist: Distribution for sampling each element
        """
        return cls.create(length_dist=IntDist.fixed(length), element_dist=element_dist)

    @classmethod
    def uniform_length(
        cls, min_len: int, max_len: int, element_dist: DataDist
    ) -> "SequenceDist":
        ...
        """
        return cls.create(
            length_dist=IntDist.uniform(min_len, max_len), element_dist=element_dist
        )
...
if __name__ == "__main__":
    ...
    seq_fixed = SequenceDist.fixed_length(
        length=5, element_dist=FloatDist.uniform(0, 1)
    ).with_seed(42)
    ...
    seq_var = SequenceDist.uniform_length(
        min_len=1, max_len=5, element_dist=FloatDist.normal(0.0, 0.1)
    ).with_seed(42)
    ...
    seq_text = SequenceDist.uniform_length(
        min_len=2, max_len=4, element_dist=TextDist.discrete(["red", "green", "blue"])
    ).with_seed(42)
    ...
    seq_test = SequenceDist.uniform_length(
        min_len=2, max_len=5, element_dist=FloatDist.uniform(0, 1)
    )
    ...
    print(
        f"  contains([0.5, 2.0, 0.3]) (elem out of range): {seq_test.contains(invalid_elem)}"
    )

    # Range
    print("\n[5] Range information")
    seq_range = SequenceDist.uniform_length(
        min_len=3, max_len=7, element_dist=FloatDist.uniform(-1, 1)
    )
    ...
    seq_ds = SequenceDist.uniform_length(
        min_len=2, max_len=4, element_dist=IntDist.uniform(0, 10)
    ).with_seed(42)
