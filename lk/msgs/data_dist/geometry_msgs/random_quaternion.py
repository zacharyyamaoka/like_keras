...
def sample_spherical_cap(
    ...
    axis_to_view, angle_to_view = axis_angle_between(
        np.array([0, 0, 1.0]), initial_view
    )
...
class RandomQuaternion(RandomType):
    ...
    def __post_init__(self):
        ...
        if self.mode == "continuous" and self.sample_fn is None:
            ...
            initial = (
                np.array(self.initial_view)
                if self.initial_view is not None
                else np.array([0, 0, 1])
            )
            hemisphere = (
                self.hemisphere_angle
                if self.hemisphere_angle is not None
                else np.pi / 2
            )
    ...
    def get_range(self) -> tuple:
        ...
        if self.mode == "fixed":
            ...
            return (
                f"hemisphere_angle={np.rad2deg(self.hemisphere_angle):.1f}deg",
    ...
    def permutation_total(self) -> int:
        ...
        if self.mode != "discrete":
            raise NotImplementedError(
                "permutation_total only supported for discrete mode"
            )
    ...
    def identity(cls) -> "RandomQuaternion":
        ...
        return cls(mode="fixed", fixed_quat=Quaternion(x=0.0, y=0.0, z=0.0, w=1.0))

    @classmethod
    def fixed(cls, x: float, y: float, z: float, w: float) -> "RandomQuaternion":
        """Create fixed Quaternion."""
        return cls(mode="fixed", fixed_quat=Quaternion(x=x, y=y, z=z, w=w))
    ...
    @classmethod
    def discrete_from_view_generator(
        cls,
        ...
        max_rotation_steps: float = np.inf,
    ) -> "RandomQuaternion":
        ...
        try:
            ...
            print(
                f"Warning: view_generator not available ({e}), using identity quaternion"
            )
            R_list = [np.eye(3)]
        ...
        return cls(mode="discrete", quaternion_list=quaternion_list)

    @classmethod
    def discrete_from_list(
        cls, quaternion_list: list[Quaternion]
    ) -> "RandomQuaternion":
    ...
    @classmethod
    def uniform_euler(
        cls, rpy_lower: list[float], rpy_upper: list[float]
    ) -> "RandomQuaternion":
        ...
        return cls(mode="continuous", sample_fn=sample_euler)
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        print(
            f"    Sample {i}: rpy={np.rad2deg(quat.rpy)}, z_deviation={np.rad2deg(angle_from_z):.1f}deg"
        )
