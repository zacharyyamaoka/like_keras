...
class RandomTransform(RandomType):
    ...
    def sample(self) -> Transform:
        ...
        return Transform(
            translation=self.translation.sample(), rotation=self.rotation.sample()
        )

    def get_range(self) -> tuple:
        """Get range information for translation and rotation."""
        return (self.translation.get_range(), self.rotation.get_range())

    @classmethod
    def identity(cls) -> "RandomTransform":
        """Create identity transform (zero translation, no rotation)."""
        return cls(
            translation=RandomVector3.zero(), rotation=RandomQuaternion.identity()
        )
