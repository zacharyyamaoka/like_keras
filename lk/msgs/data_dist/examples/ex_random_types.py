...
if __name__ == "__main__":
    ...
    # Fixed pose
    rpose = RandomPoseStamped.fixed(
        [0.5, 0.3, 0.1], [0.0, 0.0, 1.57], frame_id="conveyor"
    )
    pose = rpose.sample()
    print(
        f"Fixed: frame={pose.header.frame_id}, xyz={pose.xyz}, rpy(deg)={np.rad2deg(pose.rpy)}"
    )
    ...
    tf = rtf.sample()
    print(
        f"Fixed: translation={tf.translation.to_list()}, rpy(deg)={np.rad2deg(tf.rpy)}"
    )
    ...
    tf = rtf.sample()
    print(
        f"Random: translation={tf.translation.to_list()}, rpy(deg)={np.rad2deg(tf.rpy)}"
    )
    ...
    print(
        f"Red metal: rgba={mat.color.to_tuple()}, metallic={mat.metallic}, roughness={mat.roughness}"
    )
    ...
    mat = rmat.sample()
    print(
        f"Random: rgba={mat.color.to_tuple()}, metallic={mat.metallic:.2f}, roughness={mat.roughness:.2f}"
    )
    ...
    print(
        "  from bam.msgs.random_msgs.geometry_msgs import RandomPoseStamped, RandomVector3, RandomTransform"
    )
    print(
        "  from bam.msgs.random_msgs.visual_objects import RandomRGBA, RandomMaterial"
    )
    print("=" * 70 + "\n")
