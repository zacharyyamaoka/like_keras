...
class JointTrajectoryPoint(RosMsg):
    ...
    def lerp(
        ...
        new_point.accelerations = lerp_list(
            self.accelerations, target.accelerations, fraction
        )
        new_point.effort = lerp_list(self.effort, target.effort, fraction)
        new_point.time_from_start = lerp_list(
            self.time_from_start, target.time_from_start, fraction
        )
