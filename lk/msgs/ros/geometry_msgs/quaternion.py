...
class Quaternion(RosMsg):
    ...
    def axangle_diff(self, target: "Quaternion") -> tuple[np.ndarray, float]:
        ...
        q_diff = quaternion_multiply(
            target.to_list(), quaternion_inverse(self.to_list())
        )
...
if __name__ == "__main__":
    ...
    assert np.allclose(
        np.array(q_list), np.array(q_round_trip.to_list(euler=True)), atol=1e-6
    )
    ...
    assert np.allclose(
        np.array(q_list), np.array(q_round_trip.to_list(euler=False)), atol=1e-6
    )
