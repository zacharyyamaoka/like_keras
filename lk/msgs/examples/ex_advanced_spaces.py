...
# BAM
from lk.msgs.random_msgs import MultiBinaryDist, TextDist, SequenceDist, FloatDist, IntDist
...
def example_3_sequence():
    ...
    waypoint_dist = SequenceDist.uniform_length(
        min_len=1,
        max_len=5,
        element_dist=FloatDist.uniform(-10, 10)
    ).with_seed(42)
    
    print("  Waypoint paths:")
    for i in range(3):
        waypoints = waypoint_dist.sample()
        print(f"    Path {i+1} ({len(waypoints)} points): {[f'{w:.2f}' for w in waypoints]}")
    ...
    detection_dist = SequenceDist.uniform_length(
        min_len=0,
        max_len=5,
        element_dist=IntDist.uniform(100, 200)
    ).with_seed(42)
    ...
    for i in range(4):
        ...
        if detected_ids:
            print(f"    Frame {i+1}: Detected {len(detected_ids)} objects: {detected_ids}")
        else:
...
def example_4_sequence_of_points():
    ...
    trajectory_dist = SequenceDist.uniform_length(
        min_len=3,
        max_len=6,
        element_dist=Point.Dist.uniform(-1, 1)
    ).with_seed(42)
...
def example_5_combined():
    ...
    target_positions_dist = SequenceDist.uniform_length(
        min_len=0,
        max_len=3,
        element_dist=Point.Dist.uniform(-2, 2)
    )
...
def example_6_interface_specs():
    ...
    for name, spec in specs.items():
        ...
        if isinstance(spec, SequenceDist):
            print(f"      Length: {range_info[0]['length']} to {range_info[1]['length']}")
            print(f"      Element range: {range_info[0]['element']} to {range_info[1]['element']}")
    ...
    print(f"  Invalid buttons {invalid_buttons}: {buttons_spec.contains(invalid_buttons)}")
