...
class RandomPoseStamped(RandomType):
    ...
    def sample(self) -> PoseStamped:
        ...
        return PoseStamped(header=Header(frame_id=self.frame_id), pose=sampled_pose)
    ...
    def identity(cls, frame_id: str = "world") -> "RandomPoseStamped":
        ...
        return cls(frame_id=frame_id, pose=RandomPose.identity())

    @classmethod
    def fixed(
        cls, xyz: list[float], rpy: list[float], frame_id: str = "world"
    ) -> "RandomPoseStamped":
        """Create fixed PoseStamped from xyz and rpy."""
        return cls(frame_id=frame_id, pose=RandomPose.fixed(xyz, rpy))
...
if __name__ == "__main__":
    ...
    # Fixed
    rpose_stamped = RandomPoseStamped.fixed(
        [0.5, 0.3, 0.1], [0.0, 0.0, 1.57], frame_id="world"
    )
    pose_stamped = rpose_stamped.sample()
    print(f"\nFixed pose:")
    print(f"  frame_id: {pose_stamped.header.frame_id}")
    ...
    for i in range(3):
        pose_stamped = rpose_stamped.sample()
        print(
            f"  Sample {i}: xyz={pose_stamped.xyz}, rpy(deg)={np.rad2deg(pose_stamped.rpy)}"
        )
