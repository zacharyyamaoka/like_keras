...
def ex_continuous_hemisphere_sampling(initial_view = [0, 0, 1], hemisphere_angle=np.deg2rad(45), rotation_min=0.0, rotation_max=2*np.pi, n_samples=100):
...
def ex_discrete_sampling():
    ...
    artist.draw(arrows)
    print(f"  Rendered {len(arrows.children)} unique z-axes (from {len(R_list)} rotations)")
...
if __name__ == '__main__':
    ...
    # View, Angle, Min, Max
    configs = [[[0, 0, 1], np.deg2rad(45), 0.0, 0.0],
    ...
    ex_continuous_hemisphere_sampling(initial_view=[1, 0, 1], hemisphere_angle=np.deg2rad(180), rotation_min=0.0, rotation_max=0, n_samples=1000)
