...
class RandomFloat(RandomType):
    ...
    @classmethod
    def truncated_normal(cls, mean: float, std: float, low: float, high: float) -> 'RandomFloat':
    ...
    @classmethod
    def exponential(cls, scale: float, max_val: Optional[float] = None) -> 'RandomFloat':
    ...
    @classmethod
    def beta(cls, alpha: float, beta: float, low: float = 0.0, high: float = 1.0) -> 'RandomFloat':
    ...
    def custom(cls, sample_fn: Callable[[np.random.Generator], float],
              min_val: Optional[float] = None,
              max_val: Optional[float] = None) -> 'RandomFloat':
