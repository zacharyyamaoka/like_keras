...
class MultiBinaryDist(DataDist):
    ...
    def get_range(
        self,
    ) -> tuple[Optional[int | np.ndarray], Optional[int | np.ndarray]]:
    ...
    def single(cls, p: float = 0.5) -> "MultiBinaryDist":
        ...
        """
        return cls(sample_fn=lambda rng: int(rng.random() < p), min_val=0, max_val=1)
    ...
    def fixed(cls, value: int | np.ndarray) -> "MultiBinaryDist":
        ...
        if isinstance(value, np.ndarray):
            ...
            return cls(sample_fn=lambda rng: value, min_val=0, max_val=1)
    ...
    @classmethod
    def custom(
        cls, sample_fn: Callable[[np.random.Generator], np.ndarray | int]
    ) -> "MultiBinaryDist":
