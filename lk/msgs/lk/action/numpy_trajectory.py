...
class NumpyTrajectory:
    ...
    t_path: np.ndarray = (
        None  # [t] timestamps for path data (may be sampled if density < 1.0)
    )
    ...
    dimension_names: list[str] = (
        None  # Names of dimensions e.g., ["XYZ", "RPY", "Vec[1,0,0]"]
    )
    ...
    def inspect(self):
        ...
        print(
            f"  joint_names   : {len(self.joint_names) if self.joint_names is not None else None}"
        )
        ...
        print(
            f"  max_tau       : {self.max_tau.shape if self.max_tau is not None else None}"
        )
        print(
            f"  q_waypoint    : {self.q_waypoint.shape if self.q_waypoint is not None else None}"
        )
        print(
            f"  t_waypoint    : {self.t_waypoint.shape if self.t_waypoint is not None else None}"
        )
        print(
            f"  ik_tip_path   : {self.ik_tip_path.shape if self.ik_tip_path is not None else None}"
        )
        print(
            f"  target_link_path   : {self.target_link_path.shape if self.target_link_path is not None else None}"
        )
        print(
            f"  t_path        : {self.t_path.shape if self.t_path is not None else None}"
        )
        print(f"  path          : {self.path.shape if self.path is not None else None}")
        print(
            f"  path_d        : {self.path_d.shape if self.path_d is not None else None}"
        )
        print(
            f"  path_dd       : {self.path_dd.shape if self.path_dd is not None else None}"
        )
    ...
    def is_comparable(self, other: "NumpyTrajectory", verbose: bool = False) -> bool:
        ...
        if self.q.shape != other.q.shape:
            return fail(
                f"different position shapes ({self.q.shape} vs {other.q.shape})"
            )
        ...
        if (self.qd is not None) and (other.qd is not None):
            if self.qd.shape != other.qd.shape:
                return fail(
                    f"different velocity shapes ({self.qd.shape} vs {other.qd.shape})"
                )
        ...
        if (self.qdd is not None) and (other.qdd is not None):
            if self.qdd.shape != other.qdd.shape:
                return fail(
                    f"different acceleration shapes ({self.qdd.shape} vs {other.qdd.shape})"
                )
    ...
    def compute_cartesian_path(
        self,
        link_name: Literal["ik_tip", "target_link"] = "ik_tip",
        vec_dirs: list[np.ndarray] = None,
        in_place=False,
    ) -> CartesianPathMetrics:

        assert (
            self.t_path is not None
        ), "Time array (t_path) is required to compute cartesian path, please make sure to call add_fk_path() first"
