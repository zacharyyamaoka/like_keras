...
class PathParams:
    ...
    n_intermediate_points: int = (
        0  # Number of points to add between waypoints (straight line)
    )

    # Blending parameters (user-specified)
    blend_radius: float = 0.0  # Fillet radius in meters (must be 0 on final waypoint)
    n_intermediate_blend_points: int = (
        0  # Number of points along the arc (excluding tangent points)
    )

    # Flags set during path generation
    start_blend_point: bool = (
        False  # True if this is the start tangent point of a blend
    )
    end_blend_point: bool = False  # True if this is the end tangent point of a blend
    blend_distance: float = (
        0.0  # Distance from corner to this point (for interpolation offset)
    )
