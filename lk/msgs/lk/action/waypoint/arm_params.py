...
class ArmParams:
    ...
    sync_dir: Vector3 = field(
        default_factory=Vector3
    )  # normalized direction vector for moving target synchronization
    ...
    joint_limits: JointLimits = field(
        default_factory=JointLimits
    )  # Use typing string to avoid circular import
    ...
    def lerp(self, target: "ArmParams", fraction: float) -> "ArmParams":
        ...
        new_params.joint_impedance.kp_scale = lerp_list(
            self.joint_impedance.kp_scale, target.joint_impedance.kp_scale, fraction
        )
        new_params.joint_impedance.kd_scale = lerp_list(
            self.joint_impedance.kd_scale, target.joint_impedance.kd_scale, fraction
        )
    ...
    def difference(self, target: "ArmParams") -> dict[str, float]:
        ...
        if self.joint_impedance.kp_scale and target.joint_impedance.kp_scale:
            n_joints = min(
                len(self.joint_impedance.kp_scale), len(target.joint_impedance.kp_scale)
            )
            kp_diffs = [
                abs(
                    self.joint_impedance.kp_scale[i]
                    - target.joint_impedance.kp_scale[i]
                )
        ...
        if self.joint_impedance.kd_scale and target.joint_impedance.kd_scale:
            n_joints = min(
                len(self.joint_impedance.kd_scale), len(target.joint_impedance.kd_scale)
            )
            kd_diffs = [
                abs(
                    self.joint_impedance.kd_scale[i]
                    - target.joint_impedance.kd_scale[i]
                )
                for i in range(n_joints)
            ]
    ...
    def make_random(cls, n_joints: int = 6) -> "ArmParams":
        arm_params = cls()
        arm_params.joint_impedance.kp_scale = [
            random.uniform(0.0, 1.0) for _ in range(n_joints)
        ]
        arm_params.joint_impedance.kd_scale = [
            random.uniform(0.0, 1.0) for _ in range(n_joints)
        ]
        return arm_params
